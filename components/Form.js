...
export default function Form({ formFields=[], reqFields=[], fields={}, keys=[], submit, cancel=false }) {
	...
	const fetchData = async() => {
		...
		for (const fld of formFields) {
			...
			if(!!fields[fld] && (fields[fld].format === 'uuid' || (fields[fld].format === 'array' && !!fields[fld].opts && !!fields[fld].opts.format && fields[fld].opts.format === 'uuid'))) {
				if(!!fields[fld].opts.fetch) {
					datas[fld] = await fields[fld].opts.fetch()
				} else {
					datas[fld] = bitnook.listFormatFiles(fields[fld].opts.link)
				}
			
			// fetch data for enums
			} else if(!!fields[fld] && !!fields[fld].opts && (fields[fld].format === 'enum' || (fields[fld].format === 'array' && fields[fld].opts.format === 'enum'))) {
				if(!!fields[fld].opts.fetch) {
					datas[fld] = await fields[fld].opts.fetch()
				} else {
	...
	return (<View>
		...
		<View style={!errors.error ? {display:'none'} : {color:theme.colors.onError, backgroundColor: theme.colors.error}}>{!!errors.error ? errors.error : null}</View>
		<View style={{flexDirection:'row',justifyContent:'center'}}>
			...
			<Button
			 ...
			 onPress={() => cancel()}
			 mode='contained'
			 style={!cancel ? { display:'none'} : {margin:5}}
			 labelStyle={{}}>
				Cancel
			</Button>
		</View>
